...
static int cach_deposit_request(Player_t* player);
static void build_deck(List* deck);

//print functions:
static const char* extract_suit(uint8_t *card);
static const char* extract_rank(uint8_t* card);
static void display_cards(Player_t* player, size_t start_pos, size_t end_pos);
...
//Adds all the cards in the player\92s and dealer\92s hand to the top of the deck. 
...
static bool reset_cards(Player_t *dealer, Player_t *player, List* deck) {
	...
	while ((node = pop(dealer->_cards)) != NULL) {
		push(deck, node);
	}

	while ((node = pop(player->_cards)) != NULL) {
		push(deck, node);
	}
...
static int bet_request(Player_t *player) {
